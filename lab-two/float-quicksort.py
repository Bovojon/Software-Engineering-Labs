...
import timeit
import sys
#sys.path.insert(0, '/mnt/lovelace/software/anaconda/envs/py35/lib/python3.5/site-packages/algorithms/sorting')
#import quick_sort
from algorithms.sorting import quick_sort

def get_input_ints():
    files = ["floats-1m.dat", "floats-10m.dat"]
...
def time_quicksort():
    ...
    timeQuicksort, newArray = find_average(arrayNum)
    return timeQuicksort, newArray, arrayNum

        
def run_individually():
    timeQuicksort, newArray, arrayNum = time_quicksort()
...
def find_average(arrayNum):
    ...
    for i in range(3):
        start = time.time()
        newArray = quick_sort.sort(arrayNum)
        end = time.time()
        timeQuicksort = end - start
        arrayTimes.append(timeQuicksort)           
